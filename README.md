...
1. تفتح واجهة البرنامج وتبحث عن خدمة Rust المحلية.
...
3. إن كانت غير مثبتة أو قديمة، يتطلب التثبيت اتصالا بـ PostgreSQL وكون
   `server_control.status = '1'`، ثم تثبت الواجهة الخدمة بواسطة NSSM.
4. تولد الخدمة المفاتيح محليا، وتحفظ كل عملية محليا في مجلد بيانات النظام.
5. ترفع الخدمة السجلات المعلقة إلى PostgreSQL عند توفر الاتصال.
6. عند توفر الاتصال، تقرأ الخدمة الحالة. إذا أصبحت `0` تحفظ وضع الإيقاف
